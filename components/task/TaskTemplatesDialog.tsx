...
export default function TaskTemplatesDialog({machine_id, children}: { machine_id: string, children: React.ReactNode }) {
    ...
    return <SimpleDialog open={open} onOpenChange={openChange} trigger={children}
                         props={{maxHeight: "50%", minWidth: "20%", maxWidth: "50%"}}>
        <Dialog.Title>
            Task Templates
        </Dialog.Title>
        {taskTemplates ? <FlatList contentContainerStyle={{padding: 5, gap: 5}}
                                   renderItem={item => <TaskTemplateOverview template={item.item}/>}
                                   data={taskTemplates.concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates).concat(taskTemplates)}/> :
            <Loading/>}
    </SimpleDialog>
}